...
export default function CommercialClient({ locale = 'hu' }: Props) {
  ...
  return <div className="platform-stack">
    ...
    <section className="manufacturer-panel">
      <div className="manufacturer-toolbar"><div><strong>{t('Előfizetési és fizetős szolgáltatás kérelmek', 'Subscription & paid-service requests')}</strong><small>{t('Az admin jóváhagyás aktiválja a csomagot vagy szolgáltatást és létrehozza a számlázási ledger tételt.', 'Admin approval activates the plan or service and creates the billing-ledger charge.')}</small></div><button className="icon-button" onClick={() => void load()} aria-label={t('Frissítés', 'Refresh')}><RefreshCw size={17}/></button></div>
    ...
    <section className="manufacturer-panel">
      ...
      <div className="quote-table-wrap"><table className="quote-table"><thead><tr><th>{t('Cég', 'Company')}</th><th>{t('Típus', 'Type')}</th><th>{t('Csomag', 'Plan')}</th><th>{t('Billing', 'Billing')}</th><th>{t('Tranzakciós díj', 'Transaction fee')}</th></tr></thead><tbody>{(data.profiles || []).filter((p:any)=>p.role!=='ADMIN').map((p: any) => {
        ...
        return <tr key={p.id}><td><strong>{p.company_name || p.email}</strong><small>{p.email}</small></td><td>{p.role}{p.supplier_type ? ` · ${p.supplier_type}` : ''}</td><td><select value={p.plan || (p.role === 'BUYER' ? 'BUYER_FREE' : 'MFG_STARTER')} onChange={e => void patch({ profile_id: p.id, plan: e.target.value, subscription_status: e.target.value.endsWith('_FREE') || e.target.value.endsWith('_STARTER') ? 'FREE' : 'ACTIVE' }, `profile-${p.id}`)}>{allowed.map(plan => <option key={plan.code} value={plan.code}>{plan.code} · {en ? plan.nameEn : plan.nameHu}</option>)}</select></td><td><select value={p.subscription_status || 'FREE'} onChange={e=>void patch({profile_id:p.id,subscription_status:e.target.value},`profile-${p.id}`)}>{['FREE','REQUESTED','ACTIVE','PAST_DUE','CANCELLED','EXPIRED'].map(x=><option key={x}>{x}</option>)}</select></td><td><strong>1.5%</strong><small>{t('fix díj', 'fixed fee')}</small></td></tr>;
      })}</tbody></table></div>
    </section>
    ...
    <section className="manufacturer-panel"><div className="manufacturer-toolbar"><div><strong>Group Order {t('jutalékok', 'fees')}</strong><small>{t('A teljes MOQ célértékére számított közös beszerzési platformdíj.', 'Pooled-order platform fee calculated from the target lot value.')}</small></div><CircleDollarSign size={20}/></div><div className="quote-table-wrap"><table className="quote-table"><thead><tr><th>{t('Kampány', 'Campaign')}</th><th>{t('Célérték', 'Target value')}</th><th>Fee %</th><th>Fee</th><th>{t('Státusz', 'Status')}</th></tr></thead><tbody>{(data.groupOrders || []).map((o:any)=><tr key={o.id}><td><strong>{o.title}</strong><small>{o.status}</small></td><td>{money((Number(o.target_quantity)||0)*(Number(o.price_units_per_order_unit)||1)*(Number(o.unit_price)||0),o.currency,locale)}</td><td><strong>1.5%</strong></td><td>{money(o.platform_fee_amount,o.currency,locale)}</td><td><select value={o.commission_status||'NOT_DUE'} onChange={e=>void patch({group_order_id:o.id,commission_status:e.target.value},`go-${o.id}`)}>{['NOT_DUE','DUE','INVOICED','PAID','WAIVED'].map(x=><option key={x}>{x}</option>)}</select></td></tr>)}</tbody></table></div></section>
