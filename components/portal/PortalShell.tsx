...
export default async function PortalShell({session,children}:{session:AppSession;children:ReactNode}){
  ...
  return <div className="portal-layout"><MobilePortalMenu items={items.map(item=>({href:item.href,hu:item.hu,en:item.en}))} locale={locale} roleLabel={roleLabel} company={session.profile.company_name||session.profile.full_name} email={session.profile.email} role={session.profile.role}/><aside className="portal-sidebar"><Link href="/" className="brand portal-brand bulkpact-brand"><span className="bulkpact-mark">BP</span><strong>BulkPact</strong></Link><div className="portal-account"><span>{roleLabel}</span><strong>{session.profile.company_name||session.profile.full_name}</strong><small>{session.profile.email}</small></div><nav>{items.map(item=><Link key={item.href} href={item.href}>{item.icon}{en?item.en:item.hu}</Link>)}</nav><div className="portal-sidebar-language"><LanguageSwitcher locale={locale} inverse/></div><LogoutButton locale={locale}/></aside><div className="portal-content"><header className="portal-topbar"><div><span>{portalTitle}</span><strong>{session.profile.full_name}</strong></div><div className="portal-topbar-actions">{session.profile.role!=='ADMIN'&&<RoleSwitchButton role={session.profile.role} locale={locale} compact/>}<NotificationBell locale={locale}/><LanguageSwitcher locale={locale} compact/><Link href={action.href} className="button button-small">{action.label}</Link></div></header>{children}</div></div>;
}
