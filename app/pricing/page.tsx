...
export default async function Page(){
  ...
  const buyerPlans=en?[
    ...
    ['BUSINESS','€29 / month','Everything in Free','Advanced alerts and purchasing history','Lower eligible transaction fees','Buyer demand tools'],
    ...
    ['BUSINESS','€29 / hó','Minden a Free csomagból','Fejlett értesítések és beszerzési előzmények','Kedvezőbb jogosult tranzakciós díjak','Buyer Demand eszközök'],
  ...
  const supplierPlans=en?[
    ...
    ['PRO','€99 / month','More simultaneous campaigns','Demand insights','Featured campaign eligibility','Lower eligible platform fees'],
    ...
    ['PRO','€99 / hó','Több párhuzamos kampány','Keresleti insightok','Kiemelési lehetőség','Kedvezőbb jogosult platformdíjak'],
  ...
  return <><Header/><main>
    ...
    <section className="section"><div className="container pricing-page-stack">
      ...
      <section className="bp-revenue-model"><article><ShieldCheck/><h3>{en?'Transaction fee':'Tranzakciós díj'}</h3><p>{en?'Primary revenue can be charged only on Group Orders that actually proceed.':'A fő bevétel a ténylegesen elinduló Group Orderek után felszámított díj lehet.'}</p></article><article><Megaphone/><h3>{en?'Campaign visibility':'Kampánykiemelés'}</h3><p>{en?'Suppliers can pay for featured placement and targeted buyer reach.':'A beszállítók fizethetnek kiemelt megjelenésért és célzott vevői elérésért.'}</p></article><article><Truck/><h3>{en?'Logistics services':'Logisztikai szolgáltatások'}</h3><p>{en?'Freight, cross-dock and 3PL coordination can create additional service margin.':'Fuvar, cross-dock és 3PL koordináción további szolgáltatási margin képződhet.'}</p></article></section>
